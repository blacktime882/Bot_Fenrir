...
const DATA_URLS = {
    ...
    fissures: "https://api.tenno.tools/worldstate/pc/fissures",
};
...
async function loadData(retryCount = 0) {
    try {
        ...
        const [regionsRes, langRes, tiersRes, schedRes, fissuresRes] = await Promise.all([
            ...
            axios.get(DATA_URLS.fissures, { headers, timeout: 15000 }),
        ]);
        ...
        for (const line of schedRes.data.trim().split("\n")) {
            ...
            if (parts.length !== 2) {
                console.log(`[Data] Skipping malformed line: ${line}`);
                continue;
            }
            
            const timestamp = parseInt(parts[0].trim(), 10);
            const nodeKey = parts[1].trim();
            
            if (isNaN(timestamp) || timestamp <= 0) {
                console.log(`[Data] Skipping invalid timestamp: ${parts[0]}`);
                continue;
            }
            
            if (!nodeKey || typeof nodeKey !== 'string' || nodeKey.length === 0) {
                console.log(`[Data] Skipping invalid node key: ${parts[1]}`);
                continue;
            }
            
            schedule.push([timestamp, nodeKey]);
        }

        const fissures = fissuresRes.data.fissures?.data ?? [];
        cache = { regions: regionsRes.data, lang: langRes.data, tiers, schedule, fissures };
        console.log(`[Data] Loaded: ${Object.keys(tiers).length} tiers, ${schedule.length} slots, ${fissures.length} fissures`);
        ...
        if (retryCount < 3) {
            console.log(`[Data] Retrying in 5 seconds... (${retryCount + 1}/3)`);
...
function getCurrentFissures() {
    return cache.fissures ?? [];
}

module.exports = { loadData, translate, getTier, getNodeInfo, getFactionName, getMissionName, getLocationName, getPlanetName, getFactionKey, getCurrentAndNext, getArbitrations, getUpcomingArbitrations, getAllArbitrations, getCurrentFissures };