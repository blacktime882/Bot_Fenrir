...
def is_dark_sector_node(node_info):
    return bool(node_info and node_info.get("darkSectorData"))
...
def build_current_embed(current):
    ...
    weaknesses = FACTION_WEAKNESSES.get(faction_eng, []) if faction_eng else []
...
def main():
    ...
    if not embeds:
        embeds.append({"title": "Нет арбитражей", "description": "На ближайшее время арбитражей нет.", "color": 0x99AAB5})
